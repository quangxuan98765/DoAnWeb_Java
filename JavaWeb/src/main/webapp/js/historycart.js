...
var datasend = {action:"exportData"};
var xhr2 = new XMLHttpRequest();
xhr2.open("POST", "Historycart", true);
			...
			if (products.length == 0) {
				...
				for (let i = 0; i < DpP && (DpP * (num - 1) + i) < products.length; i++) {
					...
					if (products[page].id_sp == sp_1st[products[page].id_dh])
						productHtml += "<tr style=\"background-color: yellow;\"><th>#" + products[page].id_dh + "</th><th></th><th></th><th>" + parseInt(dh_cost[products[page].id_dh]) + "$</th><th></th><th  ><button " +((products[page].trangthai == "waiting" ? " " : "disabled style='display:none;' "))+ "class='btn-huydon' name='huydon' data-iddh='" + products[page].id_dh + "'>Hủy đơn</button><p>" + (products[page].trangthai == "waiting" ? "Đang xử lý" : "Đã xử lý") + "</p></th></tr>";
					...
					productHtml += `<a class="link-text" href="Product?Masp=` + products[page].MaSP + `">Xem chi tiết</a></div></div></td>`;
					var gia = parseInt(products[page].GiaSP) * parseInt(products[page].soluong);
					productHtml += `<td><a>` + products[page].soluong + `</a></td><td>` + gia + `$</td>`;
...
xhr2.send(JSON.stringify(datasend));

function deleteCart(iddh) {
	// Tạo đối tượng XMLHttpRequest
	var data = {action:"Delete" , id_dh:iddh};
	var xhr = new XMLHttpRequest();
	xhr.open("POST", "Historycart", true);
	xhr.onload = function() {
	}
